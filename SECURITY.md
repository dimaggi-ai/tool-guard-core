...
Tool Guard Core enforces security controls at runtime. Vulnerabilities
in it are handled with priority through the process below.
...
The **deterministic engine** (thresholds, regex, scope, SQL/path/shell
classifiers) has precise, testable guarantees — a wrong decision for a
well-formed envelope is a bug, please report it. The **LLM content
classifier** is best-effort: a single local model can be defeated by a
novel paraphrase. The shipped assertion suites cover documented attack
patterns, not all possible ones; a bypass of either layer is welcome
as a report (see the bypass template) and becomes a regression test.
...
- The `tg-proxy` HTTP service in `cmd/tg-proxy` — any path that lets a
  ...
  contract; reports of "I sent a 10 MiB body and the proxy panicked"
...
- Behavior under deliberate misconfiguration (e.g. `-fail-closed=false`
