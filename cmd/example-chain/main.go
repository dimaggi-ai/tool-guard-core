...
func main() {
	...
	traces := []domain.DecisionTrace{
		mkTrace("trc-0001", "env-0001", "", base.Add(0*time.Second), policySetHash,
			domain.DecisionAllowed, domain.ActionAllowed),
		mkTrace("trc-0002", "env-0002", "", base.Add(12*time.Second), policySetHash,
			domain.DecisionDenied, domain.ActionDenied),
		mkTrace("trc-0003", "env-0003", "", base.Add(47*time.Second), policySetHash,
			domain.DecisionAllowed, domain.ActionAllowed),
	}
	chain(traces)
...
func mkTrace(traceID, envelopeID, prev string, ts time.Time, policySetHash string, d domain.Decision, a domain.ActionTaken) domain.DecisionTrace {
