...
- The classifier prompt is deliberately framed as a routing /
  tagging task rather than "content safety" so Gemma's own safety
  filter doesn't refuse to engage; an empty response from the model
  is interpreted as `model_refused` and still fires the deny.
...
- `pkg/audit.CanonicalTraceBytes` serializes a `DecisionTrace` to a
  byte-stable representation (locked at `CanonicalTraceVersion = 1`).
...
- `Require` predicates: `top_level_kinds`, `denied_top_level_kinds`,
  `no_dynamic_sql`, `no_program_exec`, `no_modifying_cte`,
  `allowed_functions`, `denied_functions`, `no_functions`,
  `allowed_tables`, `denied_tables`, `allowed_function_classes`,
  `denied_function_classes`, `max_statements`.
...
- `path_classify` predicate with `deny_traversal`, `require_inside`,
  `deny_shell_metas` (with `include_backslash` for Linux-only mode),
  `deny_symlinks`, `resolve_symlinks`, `deny_on_resolve_failure`,
  `max_path_length`.
- `shell_classify` predicate with `argv_path_classify`, an embedded
  `path_classify` block for argv path arguments, `resolve_symlinks` for
  symmetry, and `argv_env_pattern_deny` (default catches
  `env`/`sudo`/`nice`/`ionice`/`chroot`/`doas` envelopes used to relaunch
  a different binary).
