...
event_ref="${1:?usage: verify-release-workflow-state.sh EVENT_REF MAIN_REF TAG_NAME RUN_ATTEMPT RELEASE_STATE [TAG_REF]}"
...
exec bash "${script_dir}/verify-release-tag-head.sh" "${tag_ref}" "${main_ref}" "${tag_name}" "${mode}"
