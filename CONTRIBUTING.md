...
Thanks for the interest. This is a security project — clarity, tests,
and honest commit messages matter more than throughput.
...
- New lint heuristics that map to a documented bypass class. See
  `cmd/tg/main.go:lintPolicy` for the existing seven and the comments
...
- Prefer plain Go over reflection or DSLs. The lint package
  intentionally has no rule DSL so a CTO can read the rule set in
  30 seconds.
- New external dependencies require a separate justification commit. We
  currently depend on `gopkg.in/yaml.v3` and the Go standard library.
