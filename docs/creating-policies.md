...
`tg lint -policy <file>` validates the shape and surfaces seven
heuristic warnings (see [`tg lint` heuristics](#tg-lint-heuristics)).
...
| `deny` | Returns 200 with `denied`; agent is expected to NOT call the tool |
...
conditions:
  sql_classify:
    ...
    require:
      ...
      no_modifying_cte: true          # WITH x AS (DELETE ...) deny
...
conditions:
  path_classify:
    ...
    require:
      deny_traversal: true             # any `..` segment denies
      require_inside: [/var/log/]      # path must canonicalise within
      deny_shell_metas: true           # ; | & $() ` deny
      include_backslash: true          # Linux-only mode treats \ as meta
      resolve_symlinks: true           # follow before checking
      deny_on_resolve_failure: true    # ENOENT → deny (don't fail open)
...
conditions:
  shell_classify:
    ...
    require:
      allowed_argv0: [uptime, date, whoami, df, pwd]
      argv_path_classify:              # apply path rules to argv[1:]
        deny_traversal: true
        deny_shell_metas: true
        denied_canonical_prefixes: [/etc/, /root/, /home/*/.ssh/]
...
| `rule-id-collision` | warn | Two rules share an ID |
...
A policy that fails any of these gates is REJECTED at load. There is
no silent fail-open.
