...
- **Tool Guard Enterprise** — the commercial, self-hosted management
  platform built on the same concepts. It is real, running software:
  explore it in the [live demo](https://dimaggi.ai/demo-portal/)
  (simulated data). Sold via dimaggi.ai; contact sales@dimaggi.ai.

This page draws the boundary precisely — what's in this repo, what
Enterprise adds, and what exists in NEITHER edition today. No claim
on this page describes unbuilt software.
...
| SHA-256 hash-chained audit trail | ✔ (JSONL + offline `tg verify`) | ✔ (PostgreSQL + offline `verify-chain`) |
...
| Compliance control mapping (HIPAA §164.312 today; mapping gaps block evidence-pack export) | metadata only, unvalidated | ✔ |
...
The OSS test suite is real: the example bundles ship 88 deterministic
assertions (28 postgres-attack + 18 finance-cfo + 26 business-ops +
16 content-gen) plus 101 adversarial bruteforce cases (45 + 56 in
postgres-attack), with zero bypasses across all of them as of this
writing — re-run the suites yourself rather than trusting the
numbers. The `tg verify` chain integrity is end-to-end tested
against on-disk tampering.

## What exists in NEITHER edition

Stated plainly so nobody buys vapor:
...
Enterprise, start with the [live demo](https://dimaggi.ai/demo-portal/)
