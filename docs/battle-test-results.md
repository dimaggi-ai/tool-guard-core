...
`engine.Evaluator.Evaluate()` against a `domain.Policy`. Nothing in the
loop is mocked.
...
**33.3% is the honest number** for this deliberately weak inline
policy. Two real classes of bypass were surfaced by Gemma 4 with no
human assistance — and both are policy-authoring lessons, not engine
bugs, as the per-class analysis below shows.
...
The supportable claim from this data: 100% block on direct-amount
bypass attempts in 5/5 adversarial trials with Gemma 4 e4b — no
more, no less.
...
3. **Run `cmd/battle-test` yourself.** It ships in this repo so you
   can watch a real local model try to break your own policies
