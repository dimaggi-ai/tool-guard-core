...
| `llm_classify` | LLM classification of generation requests: local Ollama (text and multimodal) or a TypeSafe System One model (text) |
...
One model instead (see [creating-policies.md](creating-policies.md)).
It asks the model to pick one label from the forbidden labels plus
`safe`, and the API returns that label as a structured field. The
Ollama-specific handling below (JSON parsing, refusal detection,
delimiters) does not apply.
