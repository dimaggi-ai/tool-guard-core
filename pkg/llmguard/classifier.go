...
func parseClassifyJSON(s string) (*ClassifyResult, error) {
	...
	// Reject anything after the first JSON value. A well-behaved
	// Format=json model emits exactly one object; a second top-level
	// object or trailing garbage is a malformed / prompt-injected
	// response and must not be accepted. dec.More() does NOT report a
	// second top-level value, so decode again and require a clean EOF.
	var trailing json.RawMessage
	if err := dec.Decode(&trailing); !errors.Is(err, io.EOF) {
		return nil, fmt.Errorf("trailing data after first JSON object — refusing ambiguous classifier output")
	}
	if out.Category == "" {
