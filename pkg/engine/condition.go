...
func evalLeaf(field string, op domain.Operator, value interface{}, fields map[string]interface{}) bool {
	...
	case domain.OpNeq:
		return !compareEq(fieldVal, value)
	case domain.OpGt:
		cmp, ok := compareNumeric(fieldVal, value)
		return !ok || cmp > 0
	case domain.OpGte:
		cmp, ok := compareNumeric(fieldVal, value)
		return !ok || cmp >= 0
	...
	case domain.OpGtField:
		...
		cmp, ok := compareNumeric(fieldVal, otherVal)
		return !ok || cmp > 0
...
// compareNumeric returns the sign of a-b and ok=false when either side is
// not numeric. Relational operators treat ok=false as fail-CLOSED (the
// condition matches) so a threshold rule on a malformed/coerced value still
// fires instead of silently passing.
