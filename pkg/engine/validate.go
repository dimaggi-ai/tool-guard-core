...
func validateOperatorValue(op domain.Operator, value interface{}, ctx string) error {
	...
	// so the rule silently never fires). Refuse it at load. An operator
	// outside the known set would also evaluate to false at runtime
	// (fail-open for a deny rule) — refuse those too.
	switch op {
	...
	default:
		return fmt.Errorf("%s: unknown operator %q — the rule would silently never fire; refuse to load", ctx, op)
	}
	return nil
}
...
func ValidatePolicy(p *domain.Policy) error {
	for i := range p.Rules {
		...
		default:
			return fmt.Errorf("policy %q: rule %q has unknown effect %q (must be one of allow|flag|escalate|deny) — refuse to load", p.PolicyID, p.Rules[i].RuleID, p.Rules[i].Effect)
		}
...
func validateCondition(c *domain.Condition, ctx string, depth int, underNot bool) error {
	...
	// missing/non-string field, model timeout, or low confidence it fires
	// (returns true) so a deny rule trips. Negating one with `not:` inverts
	// that into fail-OPEN — the malformed/adversarial input that the
	// classifier exists to catch is then treated as "did not match" and
	...
	for i := range c.Or {
		if err := validateCondition(&c.Or[i], fmt.Sprintf("%s/or[%d]", ctx, i), depth+1, underNot); err != nil {
			return err
		}
	}
	if c.Not != nil {
		if err := validateCondition(c.Not, fmt.Sprintf("%s/not", ctx), depth+1, true); err != nil {
			return err
		}
	}
