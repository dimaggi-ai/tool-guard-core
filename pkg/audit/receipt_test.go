...
func receiptTrace(t *testing.T) *domain.DecisionTrace {
	...
	if err := StampProvenance(trace, "v0.8.0-test", "sha256:"+strings.Repeat("a", 64)); err != nil {
		t.Fatal(err)
	}
	trace.TraceHash = ""
...
func TestNewDecisionReceiptRejectsIncompleteOrMalformedTrace(t *testing.T) {
	tests := []struct {
		...
		{name: "unknown canonical version", mutate: func(trace *domain.DecisionTrace) {
			trace.CanonicalVersion = "v99"
			trace.SchemaVersion = "v99"
		}},
