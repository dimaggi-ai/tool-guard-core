# Ollama agent through Tool Guard Core

An end-to-end demo: a local LLM (Gemma 4 via Ollama) drives a
real tool-use conversation through `tg-proxy`. The model reasons,
gets blocked by policy, **adapts**, gets through, and confirms the
result to the user. It runs against a local Ollama model with no external API.
...
1. **A live LLM drives the tool calls.** Gemma 4 emits JSON tool calls
   from its own reasoning, not a hardcoded script.
2. **The proxy blocks tool execution.** When the model tries to refund
   $1000, the refund-tool is never invoked — `tool.log` shows the
   "processing $X" line only for the allowed call.
3. **The model adapts to policy.** After the first denial it reasons
   about the policy note and tries a smaller amount. This is the
   failure mode policy authors care about — whether the policy holds
   when the model retries. Here it does: $499 is allowed because the
   rule is `amount > 500 → deny`.
4. **The audit chain is consistent.** Every decision is in
   `decisions.jsonl`, hash-chained, and `tg verify` walks the whole
   chain offline.
...
- **Gemma 4 e4b has variability.** It is a small local model. On some
