# postgres-attack — multi-vector LLM attack against a database and an OS shell

Five-container compose: a Postgres instance, a Linux shell tool, the
...
- Postgres is `postgres:16-alpine` with seed data; deleting a deny
  rule lets the next run actually drop the table.
- The DB tool talks to Postgres over `lib/pq`.
- The OS tool runs `sh -c <cmd>` in the os-tool container (confined to
  that container's filesystem, not the host's). The policy gate is what
  this demonstrates, not the sandbox.
- The LLM is Gemma 4 e4b over Ollama on the host. Its behavior is
  probabilistic — sometimes it tries all three attack vectors,
  sometimes it stops after the first deny. `test-policies.sh` removes
  that variance for deterministic CI.
- The proxy uses the `tg-proxy` Dockerfile from the repo root, with no
  demo-specific patches.
- The audit chain is a SHA-256 hash chain; the `sed` test above shows
  tamper detection.
