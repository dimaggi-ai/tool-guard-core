...
1. **The attack surface is unbounded**. Any free-text prompt can ask
   for any forbidden category, and a regex denylist alone is brittle —
   a paraphrase that avoids the listed terms slips past it.

2. **The risk is qualitatively different**. CSAM, deepfakes of
   politicians, copyrighted material, and voice clones are
...
3. **Off-the-shelf "model safety" is opaque and inconsistent**. The
   ...
   Tool Guard runs an operator-controlled gate outside the generator
   and writes every decision to the hash-chained audit log.
