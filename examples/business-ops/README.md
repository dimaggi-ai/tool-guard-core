...
track per-ticket state; that's the agent runtime's job, and you
have to wire it yourself -- there is no managed velocity service.
...
- A DLP / data-discovery service. The PII regex in
  ...
  silently broadens its query is out of scope for Tool Guard
  (the deterministic engine cannot inspect the resulting rowset --
  pair with database-side controls for that).
